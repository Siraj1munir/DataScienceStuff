...
class CompContNet1(FipaContractNetProtocol):
    ...
    def handle_all_proposes(self, proposes):
        ...
        # logic to select proposals by the higher available score.
        for message in proposes:
            a = """MATCH (p1:Company {name: 'Nerds Lab KHI'}) 
        MATCH (p2:Person) RETURN gds.alpha.linkprediction.preferentialAttachment(p1, p2) AS score """
            t= graph.run(a).data()
