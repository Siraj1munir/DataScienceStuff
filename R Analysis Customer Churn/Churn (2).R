# please change this path with yours path
my.env <- environment()
...
# Marginal effect
library(mfx)
logitmfx(Model,df,atmean=TRUE,robust=FALSE,clustervar1=NULL,clustervar2=NULL,start=NULL)
boxplot(df$churn,df$customer)
predict.glm(Model)
summary(df)
summary(df$churn)
...
# simple GBM machine
library(gbm)
# Wait for 2-3 min this will take a bit time
...
gbmForTesting = gbm(formula = df$churn ~ df$validation+df$revenue,
                    ...
                    shrinkage = .01,
                    n.minobsinnode = 50)
summary(gbmForTesting, plot = TRUE)
