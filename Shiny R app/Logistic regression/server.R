...
shinyServer(function(input, output, session) { 
  ...
  output$intro =renderUI({
    str1 <- 'Create logistic regression models with and without prior information.'
  ...
  output$model_explain <- renderUI({
    if(!is.null(input$datafile) & !is.null(input$iv)){
    str1 <- paste('The logistic regression model built here uses the R package MCMCpack (http://www.jstatsoft.org/v42/i09/).')
